...
while True:
    ...
    # result
    result = pd.DataFrame(
        columns=np.append(test_df.columns, np.append(resume_df.columns, ['es_skills_counter', 'nth_skills_counter'])))
    ...
    j_str = j_str + """"es_skills":{0},
                   ...
                   "resume":[""".format(str([x for x in test_df.essential_skills.values[0]]).replace("'", "\""),
                                        ...
                                        str([idfs.get(x) / np.max(list(idfs.values()))  for x in test_df.essential_skills.values[0]]).replace("'", "\""),
                                        str([idfs.get(x) / np.max(list(idfs.values()))  for x in test_df.nice_to_have_skills.values[0]]).replace("'", "\""))
